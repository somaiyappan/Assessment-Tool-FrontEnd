...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    data.map((text, index) => (
      setInitalColorQns((prevState) => 
      ({
        ...prevState, MATHS: [...prevState.MATHS, index + 1]
      }))
    ))
  ...
  const subjectNames = Object.keys(JSONData)
  ...
  for (let i = 0; i < subjectNames.length; i++) {
    subjectNamesObj[subjectNames[i]] = []

  }

  console.log(subjectNamesObj)

  const [InitalColorQns, setInitalColorQns] = useState(subjectNamesObj)
  const [AnsweredColorsQns, setAnsweredColorsQns] = useState(subjectNamesObj)
  const [NotAnsweredColorsQns, setNotAnsweredColorsQns] = useState(subjectNamesObj)
  const [MarkedColorsQns, setMarkedColorsQns] = useState(subjectNamesObj)
  ...
  const handleClickSaveEx = (position) => {
    ...
    if (valueCheck === true) {
      ...
      setAnsweredColorsQns((prevState) => ({ ...prevState, MATHS: [...prevState.MATHS, position + 1] }))
      
      


      const newIntialColorQns = InitalColorQns.filter((item) => item !== position + 1)
     setInitalColorQns ((prevState) => ({ ...prevState, MATHS: [...prevState.MATHS, newIntialColorQns] }))
    }
    else {
      setNotAnsweredColorsQns((prevState) => ({ ...prevState, MATHS: [...prevState.MATHS, position + 1] }))
      const newIntialColorQns = InitalColorQns.filter((item) => item !== position + 1)
      setInitalColorQns ((prevState) => ({ ...prevState, MATHS: [...prevState.MATHS, newIntialColorQns] }))
  ...
  const handleClickMark = (position) => {
    ...
    setMarkedColorsQns((prevState) => ({ ...prevState, MATHS: [...prevState.MATHS, position + 1] }))
    const newIntialColorQns = InitalColorQns.filter((item) => item !== position + 1)
    setInitalColorQns ((prevState) => ({ ...prevState, MATHS: [...prevState.MATHS, newIntialColorQns] }))
